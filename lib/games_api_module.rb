...
module GamesApiModule
  ...
  def involvedCompaniesRequest(game_id)
    ...
    result.each do |rs|
      ...
      else company[:type] = 'Publisher' if rs['publisher']
      end
      companies << company
    end
    puts companies
  ...
  def gamesListProcess(games_id_array)
    ...
    if games_id_array.is_a? Array
      ...
      if gamesRequest(games_id_array).first.present?
        gameCardProcess(gamesRequest(games_id_array).first)
      end
      # game_card
    end
  end
