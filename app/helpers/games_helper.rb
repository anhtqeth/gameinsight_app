...
module GamesHelper
  ...
  def card_row_data(data, rs_type)
    content = []
    content << if data.cover.nil? || data.cover == 'NA'
                 content_tag(:div, image_tag('//:0', class: 'card-img-top'),
                             class: 'card', style: 'width: 12rem;')
               else
                 content_tag(:div, image_tag(data.cover, class: 'card-img-top'),
                             class: 'card', style: 'width: 12rem;')
               end
  ...
  def card_detail(data, rs_type)
    ...
    @genres = if rs_type == 'api'
                data.genres.nil? ? 'NA' : data.genres.map(&:name).join(',')
              else
                data.game_genres.nil? ? 'NA' : data.game_genres.map(&:name).join(',')
              end
  ...
  class Carousel
    ...
    def carousel_inner(_media_data, data)
